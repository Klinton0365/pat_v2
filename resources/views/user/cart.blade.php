...
@section('content')
            ...
            <div
                        ...
                        <form id="shippingForm" method="POST" action="{{ route('checkout.process') }}">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="address" id="address" rows="3" required
                                        style="width: 100%; padding: 0.75rem 1rem; border: 2px solid #e0e0e0; border-radius: 10px; font-size: 0.95rem; transition: all 0.3s; resize: vertical;">{{ old('address', $user->address ?? '') }}</textarea>
                                </div>
                                    ...
                                    <div style="display: flex; gap: 2rem; align-items: center; flex-wrap: wrap;">
                                        <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                                            <input type="radio" name="payment_method" value="COD" required
                                                style="accent-color: #3498db; transform: scale(1.2);">
                                            <span style="font-size: 1rem; color: #2c3e50;">Cash on Delivery</span>
                                        </label>

                                        <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                                            <input type="radio" name="payment_method" value="Online" required
                                                style="accent-color: #3498db; transform: scale(1.2);">
                                            <span style="font-size: 1rem; color: #2c3e50;">Online Payment</span>
                                        </label>
                                    </div>
                                </div>
                ...
                <div style="padding: 1.5rem 2rem; border-top: 2px solid #f0f0f0; background: #f8f9fa;">
                    <div style="display: flex; gap: 1rem;">
                        ...
                        <button type="submit" id="confirmShipping"
                            style="flex: 2; padding: 1rem; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 12px; font-weight: 600; cursor: pointer; transition: all 0.2s; box-shadow: 0 4px 12px rgba(102, 126, 234, 0.3); font-size: 1rem;">
                            <i class="bi bi-lock-fill" style="margin-right: 0.5rem;"></i>Proceed to Payment
                        </button>
    ...
    document.getElementById('closeShippingModal').addEventListener('click', function () {
    ...
    {{-- @push('scripts')
        <script>
            ...
            // Confirm shipping and proceed to payment
            document.getElementById('confirmShipping').addEventListener('click', function () {
                ...
                // Show loading state
                this.disabled = true;
                this.innerHTML = '<i class="bi bi-hourglass-split" style="margin-right: 0.5rem;"></i>Processing...';

                // Send to checkout process
                fetch('{{ route("checkout.process") }}', {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            this.disabled = false;
                            this.innerHTML = '<i class="bi bi-lock-fill" style="margin-right: 0.5rem;"></i>Proceed to Payment';
                        }
                    })
                    .catch(error => {
                        console.error('Error:', error);
            ...
            // Before submitting, set selected items and coupon code into hidden fields
